...
var BookModel = books.BookModel;
var Categories = books.Categories;

var test1= Object.values(Categories).reduce( (acc, cat, idx) => {
    acc[cat] = `{ value: ${idx+1} }`;
  return acc;
}, {});
var catArr= Object.values(Categories).map( (cat, idx) => {
  return `{ ${cat}: { value: ${idx+1} }`;
});
var catSet = new Set(catArr);
//console.log(test3)

// this list must match the MyCategory in the book.js model
var Category = new GraphQLEnumType(
  {
    name: 'Category',
    values: {
